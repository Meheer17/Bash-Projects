...
echo  "$($PSQL "SELECT name FROM games INNER JOIN teams ON (games.winner_id=teams.team_id or games.opponent_id=teams.team_id) WHERE name like 'Co%' GROUP BY name")"